...
export default function EditServiceModal({
  ...
  function handleSubmit(e) {
    e.preventDefault();
    if (isLoading || isError || !id) return;
  ...
  const hasChanges = (current) => {
    return (
      initialData.name?.trim() !== current.name.trim() ||
      Number(initialData.base_price ?? 0) !== Number(current.base_price) ||
      (initialData.description ?? "").trim() !== current.description.trim()
    );
  };

  return (
    <form onSubmit={handleSubmit}>
      <h2 className="text-lg font-semibold mb-4">{t("service.edit")}</h2>
      <div>
        <label htmlFor={inputs.name.id}>{inputs.name.placeholder}</label>
        {nameErrorMsg && (
