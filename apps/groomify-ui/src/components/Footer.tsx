...
export function Footer() {
  ...
  const {email, phone, street, city, province, postal} = CONTACT_INFO;
  return (
    <footer className="bg-gray-900 text-white pt-12 pb-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
          ...
          <div>
            <h3 className="text-lg font-semibold mb-4 text-teal-400">{t('Footer.contactInfo')}</h3>
            <ul className="space-y-4">
              <li className="flex items-start">
                <MapPin className="h-5 w-5 text-teal-400 mr-3 mt-1 shrink-0" />
                <span className="text-gray-400">{`${street}, ${city}, ${province} ${postal}`}</span>
              </li>
